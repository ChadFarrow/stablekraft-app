...
/**
 * Resolve Mutton Mead & Tunes playlist tracks that need resolution
 */
...
async function resolveMMTTracks() {
    try {
        console.log('🎵 Resolving Mutton Mead & Tunes Playlist tracks...\n');
        
        // Load existing music tracks
        const musicDbPath = path.join(process.cwd(), 'data', 'music-tracks.json');
        const musicData = JSON.parse(fs.readFileSync(musicDbPath, 'utf8'));
        
        // Find MMT tracks that need resolution
        const mmtTracks = musicData.musicTracks.filter(track => 
            track.source && track.source.includes('MMT') && 
            track.needsResolution === true
        );
        
        console.log(`Found ${mmtTracks.length} MMT tracks needing resolution\n`);
        
        if (mmtTracks.length === 0) {
            console.log('✅ All MMT tracks are already resolved!');
            return;
        }
        ...
        // Process each track
        for (let i = 0; i < mmtTracks.length; i++) {
            const track = mmtTracks[i];
            const trackIndex = musicData.musicTracks.indexOf(track);
            
            console.log(`[${i + 1}/${mmtTracks.length}] Resolving track ${track.id}:`);
            ...
            if ((i + 1) % 10 === 0 || i === mmtTracks.length - 1) {
                fs.writeFileSync(musicDbPath, JSON.stringify(musicData, null, 2));
                console.log(`  💾 Saved progress (${i + 1}/${mmtTracks.length})\n`);
            }
        ...
        console.error('❌ Error resolving MMT tracks:', error);
        process.exit(1);
    }
}
...
module.exports = { resolveMMTTracks };