...
/**
 * Resolve Mutton, Mead & Music playlist tracks that need resolution
 */
...
async function resolveMMMTracks() {
    try {
        console.log('🎵 Resolving Mutton, Mead & Music Playlist tracks...\n');
        
        // Load existing music tracks
        const musicDbPath = path.join(process.cwd(), 'data', 'music-tracks.json');
        const musicData = JSON.parse(fs.readFileSync(musicDbPath, 'utf8'));
        
        // Find MMM tracks that need resolution
        const mmmTracks = musicData.musicTracks.filter(track => 
            track.source && track.source.includes('MMM') && 
            track.needsResolution === true
        );
        
        console.log(`Found ${mmmTracks.length} MMM tracks needing resolution\n`);
        
        if (mmmTracks.length === 0) {
            console.log('✅ All MMM tracks are already resolved!');
            return;
        }
        ...
        // Process each track
        for (let i = 0; i < mmmTracks.length; i++) {
            const track = mmmTracks[i];
            const trackIndex = musicData.musicTracks.indexOf(track);
            
            console.log(`[${i + 1}/${mmmTracks.length}] Resolving track ${track.id}:`);
            ...
            if ((i + 1) % 10 === 0 || i === mmmTracks.length - 1) {
                fs.writeFileSync(musicDbPath, JSON.stringify(musicData, null, 2));
                console.log(`  💾 Saved progress (${i + 1}/${mmmTracks.length})\n`);
            }
        ...
        console.error('❌ Error resolving MMM tracks:', error);
        process.exit(1);
    }
}
...
module.exports = { resolveMMMTracks };