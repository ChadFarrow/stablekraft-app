/**
 * Sync Specific Wavlake Feeds
 * Adds specific feeds to the database using their GUIDs
 */
...
// Feed GUIDs to add (all missing publisher albums)
const FEED_GUIDS_TO_ADD = [
...
async function addFeedByGuid(guid: string): Promise<boolean> {
  try {
    ...
    console.log(`   ✅ Found: ${podcastIndexFeed.title} by ${podcastIndexFeed.author}`);
    console.log(`   Episodes: ${podcastIndexFeed.episodeCount}`);
    ...
    // Create feed
    const dbFeedId = `wavlake-album-${guid.split('-')[0]}`;
    await prisma.feed.create({
      data: {
        ...
        artist: podcastIndexFeed.author,
    ...
    console.log(`   💾 Created feed: ${dbFeedId}`);
    ...
    console.log(`   ✅ Added ${episodes.length} tracks`);
    return true;
...
async function syncSpecificFeeds() {
  console.log('🚀 Syncing Specific Wavlake Feeds\n');
  ...
  console.log(`\n📋 Adding ${FEED_GUIDS_TO_ADD.length} feeds...\n`);

  let success = 0;
  let failed = 0;

  for (const guid of FEED_GUIDS_TO_ADD) {
    const result = await addFeedByGuid(guid);
    if (result) {
      success++;
    } else {
      failed++;
  ...
  console.log('📊 Summary');
  console.log('='.repeat(70));
  console.log(`Total: ${FEED_GUIDS_TO_ADD.length}`);
