...
/**
 * Resolve UpBEATs playlist tracks that need resolution
 */
...
async function resolveUpBeatsTracks() {
    try {
        console.log('🎵 Resolving UpBEATs Music Playlist tracks...\n');
        
        // Load existing music tracks
        const musicDbPath = path.join(process.cwd(), 'data', 'music-tracks.json');
        const musicData = JSON.parse(fs.readFileSync(musicDbPath, 'utf8'));
        
        // Find UpBEATs tracks that need resolution
        const upbeatsTracks = musicData.musicTracks.filter(track => 
            track.source && track.source.includes('UpBEATs') && 
            track.needsResolution === true
        );
        
        console.log(`Found ${upbeatsTracks.length} UpBEATs tracks needing resolution\n`);
        
        if (upbeatsTracks.length === 0) {
            console.log('✅ All UpBEATs tracks are already resolved!');
            return;
        }
        ...
        // Process each track
        for (let i = 0; i < upbeatsTracks.length; i++) {
            const track = upbeatsTracks[i];
            const trackIndex = musicData.musicTracks.indexOf(track);
            
            console.log(`[${i + 1}/${upbeatsTracks.length}] Resolving track ${track.id}:`);
            ...
            if ((i + 1) % 10 === 0 || i === upbeatsTracks.length - 1) {
                fs.writeFileSync(musicDbPath, JSON.stringify(musicData, null, 2));
                console.log(`  💾 Saved progress (${i + 1}/${upbeatsTracks.length})\n`);
            }
        ...
        console.error('❌ Error resolving UpBEATs tracks:', error);
        process.exit(1);
    }
}
...
if (require.main === module) {
    resolveUpBeatsTracks();
}

module.exports = { resolveUpBeatsTracks };