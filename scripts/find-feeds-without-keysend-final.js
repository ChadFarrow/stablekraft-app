...
/**
 * Final corrected script to find feeds without keysend addresses
 * 
 * This script properly handles both RSS Blue and Wavlake formats:
 * - RSS Blue: podcast:valueRecipient array with type="node"
 * - Wavlake: recipients array with type="node"
 * 
 * Usage: node scripts/find-feeds-without-keysend-final.js
 */
...
function hasKeysendInV4VValue(v4vValue) {
  ...
  try {
    ...
    if (typeof value === 'object') {
      ...
      // Check RSS Blue format: podcast:valueRecipient array
      if (value['podcast:valueRecipient'] && Array.isArray(value['podcast:valueRecipient'])) {
        return value['podcast:valueRecipient'].some(recipient => {
          // Handle RSS Blue format where attributes are in $ object
          const attrs = recipient.$ || recipient;
          return attrs.type === 'node';
        });
      }
      
      // Check Wavlake format: recipients array
      if (value.recipients && Array.isArray(value.recipients)) {
        return value.recipients.some(recipient => recipient.type === 'node');
      }
      
      // Check destinations array (another Wavlake format)
      if (value.destinations && Array.isArray(value.destinations)) {
        return value.destinations.some(destination => destination.type === 'node');
      }
    }
...
async function findFeedsWithoutKeysend() {
  try {
    console.log('🔍 Final corrected search for feeds without keysend addresses...\n');
    console.log('📝 Keysend payments identified by: type="node" in v4vValue JSON\n');
    console.log('📝 Supports both RSS Blue and Wavlake formats\n');
    ...
    // Check if Age of Reason is now properly detected
    const ageOfReason = feedsWithKeysend.find(feed => feed.title === 'Age of Reason');
    if (ageOfReason) {
      console.log('✅ Age of Reason is now properly detected as having keysend!\n');
    } else {
      console.log('❌ Age of Reason is still not detected - there may be a parsing issue\n');
    }
    ...
    if (feedsWithKeysend.length > 0) {
      ...
      feedsWithKeysend.slice(0, 3).forEach((feed, index) => {
    ...
    fs.writeFileSync('feeds-without-keysend-final.json', JSON.stringify(feedsWithoutKeysend, null, 2));
    console.log(`💾 Feeds without keysend exported to: feeds-without-keysend-final.json`);
    
    fs.writeFileSync('feeds-with-keysend-final.json', JSON.stringify(feedsWithKeysend, null, 2));
    console.log(`💾 Feeds with keysend exported to: feeds-with-keysend-final.json`);
