...
/**
 * Resolve Before the Sch3m3s playlist tracks that need resolution
 */
...
async function resolveBTSTracks() {
    try {
        console.log('🎵 Resolving Before the Sch3m3s Playlist tracks...\n');
        
        // Load existing music tracks
        const musicDbPath = path.join(process.cwd(), 'data', 'music-tracks.json');
        const musicData = JSON.parse(fs.readFileSync(musicDbPath, 'utf8'));
        
        // Find BTS tracks that need resolution
        const btsTracks = musicData.musicTracks.filter(track => 
            track.source && track.source.includes('BTS') && 
            track.needsResolution === true
        );
        
        console.log(`Found ${btsTracks.length} BTS tracks needing resolution\n`);
        
        if (btsTracks.length === 0) {
            console.log('✅ All BTS tracks are already resolved!');
            return;
        }
        ...
        // Process each track
        for (let i = 0; i < btsTracks.length; i++) {
            const track = btsTracks[i];
            const trackIndex = musicData.musicTracks.indexOf(track);
            
            console.log(`[${i + 1}/${btsTracks.length}] Resolving track ${track.id}:`);
            ...
            if ((i + 1) % 10 === 0 || i === btsTracks.length - 1) {
                fs.writeFileSync(musicDbPath, JSON.stringify(musicData, null, 2));
                console.log(`  💾 Saved progress (${i + 1}/${btsTracks.length})\n`);
            }
        ...
        console.error('❌ Error resolving BTS tracks:', error);
        process.exit(1);
    }
}
...
module.exports = { resolveBTSTracks };