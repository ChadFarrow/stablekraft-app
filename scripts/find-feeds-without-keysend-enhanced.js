...
/**
 * Enhanced script to find feeds that don't have any keysend addresses
 * 
 * This script checks:
 * 1. Feeds that don't have a v4vRecipient set at the feed level
 * 2. Feeds where none of their tracks have v4vRecipient set
 * 3. Feeds where v4vValue JSON doesn't contain keysend addresses
 * 4. Feeds that might have keysend info in RSS but not parsed to DB
 * 
 * Usage: node scripts/find-feeds-without-keysend-enhanced.js
 */
...
function hasKeysendInV4VValue(v4vValue) {
  ...
  try {
    ...
    // Check for various keysend patterns
    if (Array.isArray(value)) {
      return value.some(item => 
        item.type === 'node' || 
        item.address || 
        item.customKey === '906608' ||
        (item.address && item.address.length === 66) // Bitcoin pubkey length
      );
    }
    ...
    if (typeof value === 'object') {
      return value.type === 'node' || 
             value.address || 
             value.customKey === '906608' ||
             (value.address && value.address.length === 66);
    }
...
async function findFeedsWithoutKeysend() {
  try {
    console.log('🔍 Enhanced search for feeds without keysend addresses...\n');
    ...
    // Find feeds without keysend addresses (enhanced check)
    const feedsWithoutKeysend = [];
    const feedsWithKeysendInV4V = [];
    const feedsWithKeysendInTracks = [];
    ...
    for (const feed of allFeeds) {
      ...
      if (!hasFeedLevelKeysend && !hasFeedLevelKeysendInV4V && !hasTrackLevelKeysend) {
        ...
        if (hasFeedLevelKeysendInV4V || trackKeysendInfo.some(t => t.hasV4VKeysend)) {
          feedsWithKeysendInV4V.push({
            id: feed.id,
            title: feed.title,
            url: feed.originalUrl,
            feedLevelV4VKeysend: hasFeedLevelKeysendInV4V,
        ...
        if (hasFeedLevelKeysend || trackKeysendInfo.some(t => t.hasRecipient)) {
          feedsWithKeysendInTracks.push({
            id: feed.id,
            title: feed.title,
            url: feed.originalUrl,
    ...
    console.log(`✅ Feeds with keysend in v4vValue JSON: ${feedsWithKeysendInV4V.length}\n`);
    console.log(`✅ Feeds with keysend in v4vRecipient field: ${feedsWithKeysendInTracks.length}\n`);
    ...
    // Show some examples of feeds WITH keysend in v4vValue
    if (feedsWithKeysendInV4V.length > 0) {
      console.log('🔍 Examples of feeds WITH keysend in v4vValue JSON:\n');
      feedsWithKeysendInV4V.slice(0, 5).forEach((feed, index) => {
        console.log(`${index + 1}. ${feed.title}`);
        console.log(`   URL: ${feed.url}`);
        console.log(`   Feed-level V4V keysend: ${feed.feedLevelV4VKeysend}`);
        console.log(`   Track-level V4V keysend: ${feed.trackLevelV4VKeysend}`);
    ...
    fs.writeFileSync('feeds-without-keysend-enhanced.json', JSON.stringify(feedsWithoutKeysend, null, 2));
    console.log(`💾 Feeds without keysend exported to: feeds-without-keysend-enhanced.json`);
    
    fs.writeFileSync('feeds-with-keysend-in-v4v.json', JSON.stringify(feedsWithKeysendInV4V, null, 2));
    console.log(`💾 Feeds with keysend in v4vValue exported to: feeds-with-keysend-in-v4v.json`);
    
    fs.writeFileSync('feeds-with-keysend-in-recipient.json', JSON.stringify(feedsWithKeysendInTracks, null, 2));
    console.log(`💾 Feeds with keysend in v4vRecipient exported to: feeds-with-keysend-in-recipient.json`);
