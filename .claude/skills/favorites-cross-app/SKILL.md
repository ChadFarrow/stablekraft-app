...
## Both relay harnesses need `ws`, not whatever `WebSocket` Node has

This repo targeted Node 20 until 2026-09, where the global does not exist without a flag; it now targets Node 22 (`.nvmrc`, `node:22-alpine`), where the global is undici's and recurses with nostr-tools on a failed connect. `nostr-tools` captures the global once at module load, so `installNodeWebSocket()` (`lib/nostr/node-websocket.ts`) supplies `ws` either way. **It patches three places and all three are load-bearing**: the global, plus `useWebSocketImplementation` on **both** the CJS and ESM copies of `nostr-tools/pool` — this repo is CommonJS, so a static import resolves the CJS build while a dynamic `await import()` resolves the ESM build, two module instances with two `_WebSocket` variables. **The symptom points nowhere near the cause**: the degraded-read cases keep passing (a failed connection is indistinguishable from the degradation they assert) while only the "a relay answers" cases fail, and the probe reports **every** default relay dead. Reproduce on a newer Node with `NODE_OPTIONS=--no-experimental-websocket`. (This is also why the helper aliases the import: eslint reads any `useFoo()` call as a React hook and `next lint` errors.)
