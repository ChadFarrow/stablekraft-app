...
for (const vp of VIEWPORTS) {
  ...
  //
  // Reading the custom property off `document.documentElement` is NOT enough:
  // a custom property computes to its token list, so `--sk-safe-top` reads
  // back as the literal string `max(24px, 0px)` whether or not anything can
  // use it. Put it through a real `padding` and read the used length instead.
  const insets = await page.evaluate(() => {
    const probe = document.createElement('div');
    probe.style.cssText =
      'position:fixed;left:-9999px;padding-top:var(--sk-safe-top);padding-bottom:var(--sk-safe-bottom)';
    document.body.appendChild(probe);
    const cs = getComputedStyle(probe);
    const out = { top: parseFloat(cs.paddingTop), bottom: parseFloat(cs.paddingBottom) };
    probe.remove();
    return out;
  });
  if (insets.top !== SAFE_TOP || insets.bottom !== SAFE_BOTTOM) {
    throw new Error(`harness broken: --sk-safe-* used as ${insets.top}px/${insets.bottom}px`);
  }
...
for (const r of results) {
  ...
  // The actual report: the button at the bottom could not be reached. Note
  // that `overflows` alone is the WRONG question — the broken card did not
  // overflow *itself*, it overflowed the viewport, so `scrollHeight ===
  // clientHeight` held while half the card was off screen. Ask whether the
  // content is taller than the band the card is allowed to occupy, and require
  // an internal scroll whenever it is.
  const avail = r.innerH - SAFE_TOP - SAFE_BOTTOM;
  check(
    r.cardScrollH <= avail + 1 || r.overflows,
    `content taller than the screen scrolls inside the card (${r.cardScrollH} vs ${avail} available)`
  );
  check(
    !r.overflows || r.scrollTop > 0,
    `the card actually scrolls (scrollTop ${r.scrollTop} of ${r.cardScrollH - r.cardClientH})`
  );
  check(
