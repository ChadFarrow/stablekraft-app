...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!refresh) {
      try {
        ...
        if (dbPlaylist && dbPlaylist.tracks.length > 0) {
          ...
          const playlistAlbum = {
            id: 'b4ts-playlist', title: dbPlaylist.title, artist: 'ChadF',
            ...
            publishedAt: dbPlaylist.updatedAt.toISOString(), isPlaylistCard: true, playlistUrl: '/playlist/b4ts',
          };
    ...
    if (refresh) {
      try {
        console.log('💾 Saving B4TS playlist to database...');
        await prisma.systemPlaylist.upsert({
          where: { id: 'b4ts' },
          update: { title: 'Behind the Sch3m3s Music Playlist', artwork: artworkUrl },
          create: { id: 'b4ts', title: 'Behind the Sch3m3s Music Playlist', artwork: artworkUrl }
        });
