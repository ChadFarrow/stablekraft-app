...
export async function GET(request: Request) {
  try {
    ...
    if (!forceRefresh) {
      try {
        ...
        if (dbPlaylist && dbPlaylist.tracks.length > 0) {
          ...
          const playlistAlbum = {
            id: 'iam-playlist', title: dbPlaylist.title, artist: 'ChadF',
            ...
            publishedAt: dbPlaylist.updatedAt.toISOString(), isPlaylistCard: true, playlistUrl: '/playlist/iam',
          };
    ...
    if (forceRefresh) {
      try {
        console.log('💾 Saving IAM playlist to database...');
        await prisma.systemPlaylist.upsert({
          where: { id: 'iam' },
          update: { title: 'It\'s A Mood Music Playlist', description: 'Every music reference from the It\'s a Mood podcast', artwork: artworkUrl },
